...
namespace SingleUserTests.Booking
{
    public class BookingServiceTests
    {
        ...
        public BookingServiceTests(IBookingService bookingService, ITestDatabaseSeeder testDatabaseSeeder, AppDbContext context)
        {
            _context = context;
        ...
        {
            var exception = Record.Exception(() => _bookingService.Book(sourceId, destinationId, amount));
        ...
        [InlineData(1,2,0)]
        public void Book_AmountLessThan0_ShouldThrowException(int sourceId, int destinationId, decimal amount)
        {
            var exception = Record.Exception(() => _bookingService.Book(sourceId, destinationId, amount));
        ...
        {
            var exception = Record.Exception(() => _bookingService.Book(sourceId, destinationId, amount));
            Assert.Equal($"amount must be smaller then or equal source balance: ({1000})", exception.Message);
            Assert.IsType<ConstraintException>(exception);
            Assert.Equal(1000,_context.Ledgers.Find(sourceId)!.Balance);  
            Assert.Equal(1000,_context.Ledgers.Find(destinationId)!.Balance);  
        }
